...
($\dagger$ denotes corresponding author(s), bold denotes group members/visitors for the relevant period)

\begin{itemize}

%\item[*] \textbf{J. Mak}$^{\dagger}$, D. G. Dritschel, N. Harnik, E. Heifetz, \textbf{E. Ong} \& Y. Wang (in prep.)\\
%\textit{Baroclinic spindown in a two-layer quasi-geostrophic system:
%phenomenology and equilibration}

\item[*] R. Torres$^{\dagger}$, R. Waldman, G. Madec, C. de Lavergne, R. S\'ef\'erian \& \textbf{J. Mak} (in prep. for J. Adv. Model. Earth Syst.)\\
\textit{Energetically constrained mesoscale parameterisations in ocean global circulation models}

\item[*] \textbf{H. S. Lee}$^{\dagger}$ \textbf{J. Mak}$^{\dagger}$, D. P. Marshall \& Y. Wang (in prep. for Tellus A)\\
\textit{Over-and under-saturation of circumpolar transport controlled by residual overturning}

\item[*] J. R. Maddison$^{\dagger}$, D. P. Marshall, \textbf{J. Mak} \& K. Maurer-Song (in prep. for J. Adv. Model. Earth Syst.)\\
\textit{A two dimensional reduced order model for eddy saturation and frictional control in the Southern Ocean}

\item[*] J. Thomy, F. Sanchez, C. Prioux, S. Yau, Y. Xu, \textbf{J. Mak}, R. Sun, G. Piganeau$^{\dagger}$ \& C. C. M. Yung$^{\dagger}$ (submitted to ISME J.)\\
\textit{Phytoplankton-targeted virus enrichments link multiple viruses to host species unveiling unexpected host ranges}

\item[*] R. Liu, Y. Wang$^{\dagger}$, X. Zhai, D. Balwada \& \textbf{J. Mak} (submitted to Geophys. Res. Lett.)\\
\textit{evisiting mesoscale eddy zonal propagation: Addressing the long-wave approximation limitation for improved global estimates}

\item[23.] \textbf{J. Mak}$^{\dagger}$, N. Harnik, E. Heifetz, \textbf{G. Kumar}$^{\dagger}$ \& \textbf{E. C. Y. Ong} (accepted)\\
\textit{Edge-wave phase-shifts versus normal-mode phase-tilts in an Eady problem with a sloping boundary}\\
Physical Review Fluids

\item[22.] \textbf{X. Ruan}$^{\dagger}$, D. Couespel, M. L\'evy, \textbf{J. Mak}$^{\dagger}$ \& Y. Wang (2024)\\
...
\item[21.] \textbf{F. E. Yan}$^{\dagger}$, \textbf{J. Mak}$^{\dagger}$ \& Y. Wang (2024)\\
...
\item[20.] H. Wei, Y. Wang$^{\dagger}$ \& \textbf{J. Mak} (2024)\\
...
\item[19.] \textbf{J. Mak}$^{\dagger}$, J. R. Maddison, D. P. Marshall, \textbf{X. Ruan}, Y. Wang \& L. Yeow (2023)\\
...
\item[18.] R. Torres$^{\dagger}$, R. Waldman, \textbf{J. Mak}, \& R. S\'ef\'erian (2023)\\
...
Polar oceans seminar series, BAS, Cambridge, Nov 2023

\item[--] \textit{Inferring for a mesoscale eddy energy dissipation time-scale}\\
School of Environmental Sciences seminar, UEA, Norwich, UK, Nov 2023

\item[--] \textit{Physical and biogeochemical responses to the GEOMETRIC eddy parameterisation in an idealised ocean model}\\
LOCEAN seminar, LOCEAN Sorbonne, Paris, Nov 2023

\item[--] \textit{Inferring for a mesoscale eddy energy dissipation time-scale}\\
NOCS Physical Oceanography seminar, NOC Southampton, Oct 2023

\item[--] \textit{Mesoscale eddy parameterisation in numerical "grey zone" ocean models}\\
NEMO Systems Team meeting (virtual talk), May 2023
...
J. Adv. Model. Earth Syst.; IPCC AR6 WG1 (second draft); EGU Ocean Sci.
